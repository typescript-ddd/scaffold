...
export type GenerateAggregateOptions = {
  entityName: string;
...
export const generateAggregate = (
  ...
  const { entityName, properties = [], trackable = false } = options;
...
`
  ...
  context.addImportDeclaration({
    ...
    namedImports: [`${name}CreatedEvent`, `${name}UpdatedEvent`, `${name}DeletedEvent`],
  });
  context.addImportDeclaration({
  ...
  classDeclaration.addMethod({
    ...
    writer.writeLine(`this.applyChange(new ${name}DeletedEvent(this.id, this));`);
  });

  classDeclaration.addMethod({
    name: "create",
    isStatic: true,
    parameters: [{ name: "props", type: `Create${name}Props` }, {
      name: "id", hasQuestionToken: true, type: `${name}Id`
    }],    
    returnType: name,
  }).setBodyText((writer) => {
